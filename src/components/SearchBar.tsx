...
import type { SearchSiteEntry, SearchCountryEntry, SearchBlogEntry } from '@/lib/search-index';

interface SearchResult {
  type: 'site' | 'country' | 'blog' | 'category';
  site?: SearchSiteEntry;
...
export default function SearchBar({ className = '', searchData }: SearchBarProps) {
  ...
  const search = useCallback((q: string) => {
    ...
    const matchedSites: SearchResult[] = searchData.sites
      .filter(
        (s) =>
    ...
    const matchedCategories: SearchResult[] = searchData.categories
      ...
      {isOpen && results.length > 0 && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 max-h-[70vh] overflow-y-auto rounded-xl border border-card-border bg-[#111] shadow-2xl">
          {siteResults.length > 0 && (
            <div>
              <div className="px-4 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-widest text-text/30">Dating Sites</div>
              {siteResults.map((r) => {
                flatIndex++;
                const idx = flatIndex;
                const site = r.site!;
                return (
                  <Link key={site.slug} href={`/site/${site.slug}`} onClick={() => { setIsOpen(false); setQuery(''); }}
                    className={`flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gold/10 ${idx === activeIndex ? 'bg-gold/10' : ''}`}>
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gold/5 text-xl">{site.logo}</span>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="truncate text-sm font-medium text-text">{site.name}</span>
                        <span className="shrink-0 rounded-full bg-gold/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-gold/70">{site.category}</span>
                      </div>
                      <span className="text-xs text-text/40">{site.bestFor}</span>
                    </div>
                    <span className="shrink-0 font-serif text-sm font-bold text-gold">{site.score.toFixed(1)}</span>
                  </Link>
                );
              })}
            </div>
          ...
          {countryResults.length > 0 && (
            <div>
              ...
              {countryResults.map((r) => {
                ...
                return (
                  <Link key={country.slug} href={`/country/${country.slug}`} onClick={() => { setIsOpen(false); setQuery(''); }}
                    className={`flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gold/10 ${idx === activeIndex ? 'bg-gold/10' : ''}`}>
          ...
          {categoryResults.length > 0 && (
            <div>
              <div className="px-4 pt-3 pb-1 text-[11px] font-semibold uppercase tracking-widest text-text/30">Categories</div>
              {categoryResults.map((r) => {
                flatIndex++;
                const idx = flatIndex;
                return (
                  <Link key={r.category} href={`/category/${toSlug(r.category!)}`} onClick={() => { setIsOpen(false); setQuery(''); }}
                    className={`flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gold/10 ${idx === activeIndex ? 'bg-gold/10' : ''}`}>
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gold/5 text-lg">📂</span>
          ...
          {blogResults.length > 0 && (
            <div>
              ...
              {blogResults.map((r) => {
                ...
                const post = r.blog!;
                return (
                  <Link key={post.slug} href={`/blog/${post.slug}`} onClick={() => { setIsOpen(false); setQuery(''); }}
                    className={`flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-gold/10 ${idx === activeIndex ? 'bg-gold/10' : ''}`}>
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gold/5 text-lg">📝</span>
                    <div className="min-w-0 flex-1">
